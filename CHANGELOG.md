...
- Paperclip: vendored `hermes-paperclip-adapter` **0.3.2** probes `HERMES_CLI`, `HERMES_AGENT_REPO` layout (`venv/.venv/bin/hermes`), `~/.local/bin/hermes`, and Homebrew paths when the Paperclip server has a minimal PATH (fixes Hermes adapter env check under IDE/GUI launches).
