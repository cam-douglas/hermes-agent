...
- Paperclip: vendored `hermes-paperclip-adapter` **0.3.3** probes `HERMES_CLI`, `HERMES_AGENT_REPO` layout (`venv/.venv/bin/hermes`), `~/.local/bin/hermes`, and Homebrew paths when the Paperclip server has a minimal PATH (fixes Hermes adapter env check under IDE/GUI launches). The env test no longer **fails** on macOS’s stale `/usr/bin/python3` when the Hermes CLI shim already works.
