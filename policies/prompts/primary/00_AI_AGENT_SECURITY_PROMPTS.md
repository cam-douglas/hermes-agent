...
Primary objectives:
1. preserve host/guest separation;
2. preserve non-admin, fail-closed operation;
3. keep all access scoped to explicit guest workspace roots only;
...
You must check:
- host/guest separation signals where detectable
...
- no password manager
- no sync
- downloads only into guest workspace
...
10. recommend rollback to known-good VM snapshot if integrity is in doubt
