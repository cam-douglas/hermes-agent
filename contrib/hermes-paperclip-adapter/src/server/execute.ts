...
export async function execute(
  ...
  const configuredCli = cfgString(config.hermesCommand) || HERMES_CLI;
  const inv = await resolveHermesInvocation(configuredCli);

  // ── Build command args ─────────────────────────────────────────────────
  // Use -Q (quiet) to get clean output: just response + session_id line
  const useQuiet = cfgBoolean(config.quiet) !== false; // default true
  const args: string[] = [...inv.argsPrefix, "chat", "-q", prompt];
  ...
  const result = await runChildProcess(ctx.runId, inv.command, args, {
    cwd,
    env,
    timeoutSec,
    graceSec,
