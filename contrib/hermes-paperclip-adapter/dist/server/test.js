...
import { resolveHermesInvocation } from "./resolve-hermes.js";
...
// Checks
// ---------------------------------------------------------------------------
async function checkCliVersion(inv) {
    try {
        const { stdout } = await execFileAsync(inv.command, [...inv.argsPrefix, "--version"], {
            timeout: 10_000,
        });
...
export async function testEnvironment(ctx) {
    ...
    const checks = [];
    // 1. CLI resolvable (PATH shim or python -m hermes_cli.main)
    let inv;
    try {
        inv = await resolveHermesInvocation(command);
    }
    catch (err) {
        const e = err;
        if (e.code === "ENOENT") {
            checks.push({
                ...
                hint: "Install Hermes Agent (`pip install hermes-agent`) or ensure `python3 -m hermes_cli.main --version` works (venv installs often omit the `hermes` shim from the Paperclip process PATH).",
            ...
            checks.push({
                level: "error",
                message: `Could not run Hermes CLI: ${e?.message ?? String(err)}`,
                hint: "Check that Hermes is installed and the configured `hermesCommand` path is correct.",
                code: "hermes_cli_resolve_failed",
            });
        }
        return {
    ...
    if (inv.argsPrefix.length > 0) {
        checks.push({
            level: "info",
            message: `Using Hermes via \`${inv.command} ${inv.argsPrefix.join(" ")}\` (no \`hermes\` on PATH)`,
            code: "hermes_python_module_invocation",
        });
    }
    // 2. CLI version
